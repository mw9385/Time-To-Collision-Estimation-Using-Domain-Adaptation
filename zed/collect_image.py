...
#             cv2.waitKey(1)
            ...
            if len(init_buffer) > 40 and episode_start and not episode_end:
                if count:
                    time.sleep(1.5)
                    image_buffer = []
                count = False
                ...
                temp_image = cv2.resize(temp_image, dsize=(128,128), interpolation=cv2.INTER_AREA)
            ...
            elif len(init_buffer) > 40 and not episode_start and episode_end:
                print('Not recording \n')
                if not count:
